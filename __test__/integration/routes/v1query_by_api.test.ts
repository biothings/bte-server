...
describe("Testing /v1/smartapi/{smartapi_id}/query endpoints", () => {
  ...
  test.skip("Query to non-Text Mining KPs should have id resolution turned on", async () => {
    ...
    mockedAxios.post.mockResolvedValue({ data: JSON.parse(fs.readFileSync(sri_path, { encoding: "utf8" })) });
    const mychem_path = path.resolve(__dirname, "../../data/api_results/mychem_query.json");
    // @ts-expect-error axios mocking is bad 
    mockedAxios.mockResolvedValue({ data: JSON.parse(fs.readFileSync(mychem_path, { encoding: "utf8" })) });
    ...
    // })
    const query = JSON.parse(
      fs.readFileSync(path.join(example_folder, "serviceprovider/mychem.json"), { encoding: "utf8" }),
    );
    await request(app)
      .post(`/v1/smartapi/${myChemAPI}/query`)
      .send(query)
